...
/// in the `null_values_to_replace` list.
///
/// ### Arguments
///
/// * `dataframe`: The input DataFrame to be processed. Must contain columns of DataType::String.
/// * `null_values_to_replace`: A slice of string literals (e.g., `&["NA", "N/A", ""]`)
...
pub fn replace_strings_with_null(
    ...
    dataframe: DataFrame,
    null_values_to_replace: &[&str],
) -> Result<DataFrame, PolarsError> {
    if null_values_to_replace.is_empty() {
        return Ok(dataframe);
    }
    ...
    let null_values_series =
        Series::new("null_vals".into(), null_values_to_replace).cast(&DataType::String)?; // Cast to match target column type
