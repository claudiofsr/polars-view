...
use crate::{
    ...
    PolarsViewError,
    PolarsViewResult,
    // Import Sort types relevant to this module
    SortBy,
    SortableHeaderRenderer, // Trait for header rendering
...
/// - **`filters`**: Configuration used for *loading* the data (path, delimiter, SQL query, etc.). **Does NOT contain sorting information.**
...
/// - Updates (load, format, sort) typically create *new* `DataFrameContainer` instances via async methods.
///
/// ## Interaction with `layout.rs`:
/// - `PolarsViewApp` holds the current state as `Option<Arc<DataFrameContainer>>`.
/// - UI actions trigger async methods here (`load_data`, `update_format`, `apply_sort`).
/// - Async methods return `PolarsViewResult<DataFrameContainer>` via a channel.
...
impl Default for DataFrameContainer {
    ...
    fn default() -> Self {
        ...
        DataFrameContainer {
            ...
            filters: Arc::new(DataFilters::default()), // Filters has no sort field
...
impl DataFrameContainer {
    /// Asynchronously loads data based on `DataFilters`, applies transformations,
    /// and returns a **new**, unsorted `DataFrameContainer`.
    ///
    /// Triggered by `layout.rs` on file open, drag-drop, or filter/SQL changes from the side panel.
    ///
    /// ## Execution Steps & State Updates:
    /// 1.  Validate Path.
    /// 2.  Read Data (using `filters`, may update delimiter).
    /// 3.  Apply SQL (if `filters.apply_sql` is true, then reset it to `false`).
    /// 4.  Remove Null Columns (if `filters.remove_null_cols` is true).
    /// 5.  Update `filters.schema` with the final schema.
    /// 6.  Create **new** `DataFrameContainer`:
    ///     - `df` and `unsorted_df` point to the *same* Arc of the loaded/processed DataFrame.
    ///     - `filters` Arc contains the loading/query settings used.
    ///     - `format` Arc uses the provided format.
    ///     - `sort` is initialized as an empty `Vec::new()`.
    /// 7.  Return `Ok(new_container)`.
    pub async fn load_data(mut filters: DataFilters, format: DataFormat) -> PolarsViewResult<Self> {
        // --- 1. Path Validation ---
        if !filters.absolute_path.exists() {
            tracing::error!("load_data: File not found: {:?}", filters.absolute_path);
            return Err(PolarsViewError::FileNotFound(filters.absolute_path.clone()));
        }
        tracing::debug!(
            "load_data: Loading from {:?}. Filters: {:#?}, Format: {:#?}",
        ...
        tracing::debug!(
            "load_data: Initial load complete. Dims: {}x{}, Ext: {:?}, Delimiter: '{}'",
            df.height(),
            df.width(),
            extension,
        ...
        if filters.apply_sql {
            tracing::debug!("load_data: Applying SQL: '{}'", filters.query);
            let mut ctx = SQLContext::new();
            ctx.register(&filters.table_name, df.lazy()); // Register current df as table.
            df = ctx.execute(&filters.query)?.collect()?; // Execute query, collect result back to df.
            filters.apply_sql = false; // Reset flag *after* successful execution
            tracing::debug!(
        ...
        // --- 4. Null Column Removal (if requested) ---
        if filters.remove_null_cols {
            let initial_width = df.width();
            df = remove_null_columns(&df)?; // Call helper from polars.rs
            tracing::debug!(
                "load_data: Null columns removed. Width change: {} -> {}",
                initial_width,
                df.width()
            );
        }

        // --- 5. Update Schema in Filters ---
        filters.schema = df.schema().clone();
        tracing::debug!("load_data: Schema updated within filters.");

        // --- 6. Create Arcs & Container ---
        let final_df_arc = Arc::new(df);

        tracing::debug!("load_data: Success. Returning new container (unsorted).");

        Ok(Self {
            df: final_df_arc.clone(),  // Current view is the loaded data initially
            unsorted_df: final_df_arc, // Store reference to this post-load state
            extension: Arc::new(extension),
            filters: Arc::new(filters), // Filters used for loading
            format: Arc::new(format),   // Format settings provided
            sort: Vec::new(),           // Data is unsorted immediately after load
        })
    }
    ...
    /// - Reuses existing Arcs for `df`, `unsorted_df`, `extension`, `filters`, `sort` via cloning.
    /// - Incorporates the *new* `format` Arc.
    pub async fn update_format(
        data_container: Arc<DataFrameContainer>, // Arc to current state
    ...
    /// Asynchronously creates a *new* `DataFrameContainer` with the `df` sorted according
    /// to the provided `new_sort_criteria`.
    ///
    /// Triggered by `layout.rs` after a user clicks a sortable header, resulting in new criteria.
    /// Handles multi-column sorting based on the order and direction in `new_sort_criteria`.
    ...
    ///     a. Prepare Polars multi-sort options from `new_sort_criteria`.
    ///     b. Call `data_container.df.sort()` using the *currently displayed* df as input.
    ...
    pub async fn apply_sort(
        ...
        // 3a. Prepare sort parameters
        let column_names: Vec<PlSmallStr> = new_sort_criteria
            .iter()
            .map(|sort| sort.column_name.clone().into())
            .collect();
        ...
        // NOTE: This sorts the potentially *already sorted* df based on the *new cumulative* criteria.
        // If performance becomes an issue with many cumulative sorts, one might consider sorting
        // from `unsorted_df` each time, but that discards intermediate stable sort results.
        // Current approach matches user expectation of refining the existing sort order.
    ...
    /// Reads the current sort state (`self.sort`). On click, calculates the next
    /// sort state for the clicked column, modifies a *cloned* sort criteria `Vec`,
    /// and signals this *new `Vec`* back via the `sort_signal` output parameter.
    ///
    /// # Arguments
    ...
    fn render_table_header(
        ...
        for column_name in self.df.get_column_names() {
            table_row.col(|ui| {
                // 1. Determine current interaction state and sort index for THIS column from `self.sort`.
                let (current_interaction_state, sort_index) = self
                    ...
                    .map_or((HeaderSortState::NotSorted, None), |index| {
                        let state = if self.sort[index].ascending {
                ...
                // 2. Render the sortable header widget.
                ...
                if response.clicked() {
                    ...
                    match next_interaction_state {
                        ...
                        HeaderSortState::Ascending => {
                            if let Some(pos) = current_pos {
                                new_sort_criteria[pos].ascending = true;
                            } else {
                        ...
                        HeaderSortState::Descending => {
                            if let Some(pos) = current_pos {
                                new_sort_criteria[pos].ascending = false;
                            } else {
                                new_sort_criteria.push(SortBy {
                                    column_name: column_name_string,
                                    ascending: false,
