...
use polars::prelude::*;
use regex::Regex; // Add the regex crate for upfront validation
...
pub fn normalize_float_strings_by_regex(
    ...
    // --- 0. Validate Regex Pattern Syntax and Format ---
    let compiled_regex = if regex_pattern != "*" {
        // Check format first
        ...
        match Regex::new(regex_pattern) {
            Ok(re) => Some(re),
            Err(e) => {
                // Return specific error for invalid regex syntax
        ...
        None // No need to compile for "*"
    };

    // --- 1. Identify Columns to Transform and Potential Errors ---
