...
pub fn replace_values_with_null(
    ...
    // 1. Create a Polars Series containing the *strings* to be treated as null markers.
    // Series<String> with height N (where N is the length of the list)
    let series = Series::new("null_vals".into(), null_value_list);
