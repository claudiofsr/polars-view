...
impl FileMetadata {
    ...
    /// Adds copy-to-clipboard functionality on left-click of the column name.
    pub fn render_schema(&self, ui: &mut Ui) {
        ...
        for (index, (name, dtype)) in self.schema.iter().enumerate() {
            ...
            // Check if the header was clicked (specifically with the left mouse button).
            if header_response
                .header_response
                .clicked_by(egui::PointerButton::Secondary)
            {
                // If the left mouse button was clicked, copy the column name to the clipboard.
