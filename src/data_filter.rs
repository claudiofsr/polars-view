...
impl DataFilter {
    ...
    async fn read_csv_data(&self) -> PolarsViewResult<(DataFrame, Option<u8>)> {
        ...
        for delimiter in delimiters_to_try {
            ...
            if let Ok(schema) = self.attempt_csv_parse_structure(delimiter).await {
    ...
    async fn attempt_csv_parse_structure(&self, delimiter: u8) -> PolarsViewResult<Arc<Schema>> {
        ...
        if data_frame.width() <= min_expected_cols_on_success {
            tracing::warn!(
...
12345;30.7;ghi";
        ...
        let schema = filter.attempt_csv_parse_structure(delimiter as u8).await?;
