use crate::{
    ...
    SortState, UniqueElements, sql_commands,
};
use egui::{
...
/// Holds configuration parameters and state related to data loading, querying, and processing.
/// This struct acts as the central source of truth for how data should be loaded from a file
/// and subsequently transformed (e.g., via SQL). It also includes UI-related state like sorting.
///
/// Instances of `DataFilters` are passed around, notably:
/// - Created initially from `Arguments` in `main.rs`.
/// - Cloned and modified within the UI (`render_query`) to capture user changes.
/// - Passed (by value or within `Arc`) to `DataFrameContainer` methods (`load_data`, `sort`)
///   to specify the parameters for those operations.
/// - Stored in `PolarsViewApp` (`layout.rs`) as `applied_filters` to track the last applied state
///   and detect changes made in the UI.
#[derive(Debug, Clone, PartialEq)]
pub struct DataFilters {
    /// The canonical, absolute path to the data file.
    pub absolute_path: PathBuf,
    /// The name assigned to the loaded DataFrame for use in SQL queries (e.g., `FROM <table_name>`).
    pub table_name: String,
    /// The character used to separate columns in a CSV file. Stored as `String` to allow user edits.
    pub csv_delimiter: String,
    /// The schema (column names and data types) of the *currently loaded or last successfully loaded* DataFrame.
    /// Wrapped in `Arc` for efficient sharing, avoiding deep copies when filters are cloned or passed around.
    /// This schema is used, for example, by `sql_commands` to generate relevant examples. It's updated
    /// *after* successful data loading in `DataFrameContainer::load_data`.
    pub schema: Arc<Schema>,
    /// The maximum number of rows to scan when inferring the schema for CSV, Json, and NDJson files.
    pub infer_schema_rows: usize,
    /// The SQL query string entered by the user to be executed against the loaded DataFrame.
    pub query: String,
    /// A flag indicating whether the `query` should be executed.
    /// - Set to `true` if the `query` field differs from `DEFAULT_QUERY` during initial load from `Arguments`.
    /// - Set to `true` in `render_query` if any filter setting (query, delimiter, schema rows, etc.) is changed.
    /// - Checked by `DataFrameContainer::load_data` to decide whether to run the SQL query.
    /// - Reset to `false` in `DataFrameContainer::load_data` after the query is applied, preventing re-application on subsequent sorts or format changes.
    pub apply_sql: bool,
    /// The current sorting state (column name and direction: Ascending, Descending, or NotSorted).
    /// - `Option<Arc<SortState>>`: `Option` allows for no sort, `Arc` allows cheap cloning and sharing.
    /// - Updated by `render_table_header` in `container.rs` when a sort button is clicked.
    /// - Read by `DataFrameContainer::sort` to perform the actual sorting.
    pub sort: Option<Arc<SortState>>,
    /// Flag to control whether columns containing only null values should be removed after loading.
    pub remove_null_cols: bool,
    /// A comma-separated string of values to be interpreted as nulls during CSV parsing.
    /// Parsed into `Vec<PlSmallStr>` within `attempt_read_csv`.
    pub null_values: String,
}

impl Default for DataFilters {
    /// Creates a default `DataFilters` instance with sensible initial values.
    fn default() -> Self {
        DataFilters {
            absolute_path: PathBuf::new(),     // Empty path initially.
            table_name: "AllData".to_string(), // Default table name for SQL.
            csv_delimiter: DEFAULT_CSV_DELIMITER.to_string(), // Default CSV delimiter.
            schema: Schema::default().into(),  // Empty schema initially.
            infer_schema_rows: 200,            // Default rows for schema inference.
            query: DEFAULT_QUERY.to_string(),  // Default SQL query (select all).
            apply_sql: false,                  // Don't apply SQL by default.
            sort: None,                        // No sorting initially.
            remove_null_cols: false,           // Don't remove null columns by default.
            null_values: NULL_VALUES.to_string(), // Default null value representations.
        }
    }
}
...
impl DataFilters {
    ...
    pub fn new(args: &Arguments) -> PolarsViewResult<Self> {
        ...
        Ok(DataFilters {
            ...
            ..Default::default() // Use defaults for `schema`, `infer_schema_rows`, `sort`.
        })
    }

    /// Sets the data source path, converting it to an absolute, canonical path.
    /// Called during file open/drag-drop in `layout.rs`.
    ///
    /// ### Arguments
    /// * `path`: The new `Path` to the data file.
    ///
    /// ### Returns
    /// `Ok(())` on success, `Err(PolarsViewError::Io)` if canonicalization fails.
    ...
    /// Gets the file extension from the `absolute_path` in lowercase.
    /// Returns `None` if there's no extension.
    /// Uses the `PathExtension` trait.
