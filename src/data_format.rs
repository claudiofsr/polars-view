...
impl Default for DataFormat {
    ...
    fn default() -> Self {
        DataFormat {
            ...
            auto_col_width: false,                  // Default to faster fixed-width columns.
...
impl DataFormat {
    ...
    fn render_alignment_panel(&mut self, ui: &mut Ui) {
        ...
        ui.collapsing("Data Types", |ui| {
            ...
            Grid::new("align_grid")
                ...
                .striped(true)
                .show(ui, |ui| {
                    // Render rows for relevant DataTypes, passing mutable map.
                    Self::show_alignment_row(ui, &DataType::Float64, &mut self.alignments);
                    Self::show_alignment_row(ui, &DataType::Float32, &mut self.alignments);
                    Self::show_alignment_row(ui, &DataType::Int64, &mut self.alignments);
                    Self::show_alignment_row(ui, &DataType::Int32, &mut self.alignments);
                    Self::show_alignment_row(ui, &DataType::Int16, &mut self.alignments);
                    Self::show_alignment_row(ui, &DataType::Int8, &mut self.alignments);
                    Self::show_alignment_row(ui, &DataType::UInt64, &mut self.alignments);
                    Self::show_alignment_row(ui, &DataType::UInt32, &mut self.alignments);
                    Self::show_alignment_row(ui, &DataType::UInt16, &mut self.alignments);
                    Self::show_alignment_row(ui, &DataType::UInt8, &mut self.alignments);
                    Self::show_alignment_row(ui, &DataType::Date, &mut self.alignments);
                    Self::show_alignment_row(ui, &DataType::Time, &mut self.alignments);
                    Self::show_alignment_row(ui, &DataType::Boolean, &mut self.alignments);
                    Self::show_alignment_row(ui, &DataType::Binary, &mut self.alignments);
    ...
    ///
    /// Uses `ui.radio_value`, which directly reads and *mutates* the `alignments` `HashMap`
    /// entry for the given `data_type` when the user clicks a different radio button.
    fn show_alignment_row(
        ui: &mut Ui,
        data_type: &DataType,
        alignments: &mut HashMap<DataType, Align>, // Modified here
    ) {
