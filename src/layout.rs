use crate::{
    ...
    PolarsViewError, PolarsViewResult, Settings, SortBy, open_file, save, save_as,
};
...
pub struct PolarsViewApp {
    /// Holds the currently loaded data and its associated view state (`DataFrameContainer`).
    ...
    /// Metadata extracted from the currently loaded file.
        ...
        match self.runtime.block_on(open_file()) {
            Ok(path) => {
                ...
                if let Err(error) = self.applied_filters.set_path(&path) {
                    ...
                    tracing::debug!(
                        "handle_open_file: Triggering load_data for path: {:#?}",
                        self.applied_filters.absolute_path
                    );
        ...
        TopBottomPanel::bottom("bottom_panel").show(ctx, |ui| {
            ui.horizontal(|ui| {
                ...
                if let Some(container) = &self.data_container {
                    ...
                    ui.label(format!(
                        "File: {}",
                        container.filters.absolute_path.to_string_lossy()
                    ));
                    ui.separator();
                ...
                match &self.data_container {
                    Some(container) => {
                        ...
                        if let Some(new_criteria) = opt_new_sort_criteria {
                            ...
                            let future = DataFrameContainer::apply_sort(
        ...
        if let Some(dropped_file) = ctx.input(|i| i.raw.dropped_files.first().cloned()) {
            if let Some(path) = &dropped_file.path {
                ...
                if let Err(error) = self.applied_filters.set_path(path) {
                    ...
                    tracing::debug!("Drag-n-drop: Triggering load_data for path: {:#?}", path);
